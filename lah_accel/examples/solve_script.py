...
from lah_accel.scs_problem import SCSinstance
import pdb
import cvxpy as cp
...
def nonneg_ls_setup_script(b_mat, theta_mat, A, output_filename, box=None):
    # def solve_many_probs_cvxpy(A, b_mat, lambd):
    """
    solves many lasso problems where each problem has a different b vector
    """
    ...
    N = b_mat.shape[0]
    z, b_param = cp.Variable(n), cp.Parameter(m)
    prob = cp.Problem(cp.Minimize(.5 * cp.sum_squares(np.array(A) @ z - b_param)), constraints=[z >= 0])
    z_stars = jnp.zeros((N, n))
    objvals = jnp.zeros((N))
    solve_times = np.zeros(N)
    for i in range(N):
    ...
    jnp.savez(
        output_filename,
        thetas=jnp.array(theta_mat),
        q_mat=jnp.array(theta_mat), #jnp.array(b_mat),
        z_stars=z_stars,
    )
