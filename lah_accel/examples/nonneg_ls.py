...
from lah_accel.examples.solve_script import nonneg_ls_setup_script
import os
...
def run(run_cfg, model='lah'):
    ...
    static_dict = dict(A=A, accel=run_cfg.accel)

    # we directly save q now
    static_flag = True
    if model == 'lah':
        algo = 'lah_nonneg_gd_accel'
    elif model == 'l2ws':
        algo = 'nonneg_gd'
...
def setup_probs(setup_cfg):
    ...
    nonneg_ls_setup_script(c_mat, theta_mat, A, output_filename)
    # jnp.savez(
    #     output_filename,
...
def solve_many_probs_cvxpy(A, b_mat, lambd):
    """
    solves many nonneg ls problems where each problem has a different b vector
    """
    m, n = A.shape
    N = b_mat.shape[0]
