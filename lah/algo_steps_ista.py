...
def k_steps_eval_fista_l2ws(k, z0, q, ista_step, lambd, A, supervised, z_star, jit):
    iter_losses = jnp.zeros(k)
    obj_diffs = jnp.zeros(k)
    z_all_plus_1 = jnp.zeros((k + 1, z0.size))
    z_all_plus_1 = z_all_plus_1.at[0, :].set(z0)
    fp_eval_partial = partial(fp_eval_fista_l2ws,
                              supervised=supervised,
                              z_star=z_star,
                              A=A,
                              b=q,
...
def k_steps_train_fista_l2ws(k, z0, q, ista_step, lambd, A, supervised, z_star, jit):
    iter_losses = jnp.zeros(k)
    obj_diffs = jnp.zeros(k)
    z_all_plus_1 = jnp.zeros((k + 1, z0.size))
    z_all_plus_1 = z_all_plus_1.at[0, :].set(z0)
    fp_eval_partial = partial(fp_train_fista,
                              supervised=supervised,
                              z_star=z_star,
                              A=A,
                              b=q,
...
def fp_eval_fista_l2ws(i, val, supervised, z_star, A, b, lambd, ista_step):
...
def k_steps_train_lah_fista(k, z0, y0, q, params, lambd, A, supervised, z_star, jit):
    iter_losses = jnp.zeros(k)

    fp_train_partial = partial(fp_train_lah_fista,
                               supervised=supervised,
                               z_star=z_star,
                               lambd=lambd,
                               A=A,
...
def fp_train_lah_fista(i, val, supervised, z_star, lambd, A, c, ista_steps):
    z, y, t, loss_vec = val
...
def k_steps_eval_lah_fista(k, z0, q, params, lambd, A, supervised, z_star, jit):
    ...
    fp_eval_partial = partial(fp_eval_lah_fista,
                              supervised=supervised,
                              z_star=z_star,
                              lambd=lambd,
                              A=A,
